...
from events.stdDAO import onclick_add, onclick_save, onclick_delete
...
def main():
    ...
    frame1.pack(side="top", pady=5, anchor="nw", fill="x")
    label1 = tk.Label(frame1, text="학과")
    label1.pack(side="left", padx=5)
    combo1 = ttk.Combobox(frame1, width=16, values=majors1)
    combo1.current(0)
    ...
    frame3_1.pack(side="top", pady=5, anchor="w")
    label5 = tk.Label(frame3_1, text="학번", width=6, anchor="w")
    label5.pack(side="left", padx=5)
    entry5 = tk.Entry(frame3_1, width=12, state="disabled")
    entry5.pack(side="left", padx=5)
    label6 = tk.Label(frame3_1, text="이름", width=6, anchor="e")
    label6.pack(side="left", padx=5)
    entry6 = tk.Entry(frame3_1, width=12, state="disabled")
    entry6.pack(side="left", padx=5)

    frame3_2 = tk.Frame(frame3)
    frame3_2.pack(side="top", pady=5, anchor="w")
    label7 = tk.Label(frame3_2, text="이메일", width=6, anchor="w")
    label7.pack(side="left", padx=5)
    entry7 = tk.Entry(frame3_2, width=34)
    entry7.pack(side="left", padx=5)

    frame3_3 = tk.Frame(frame3)
    frame3_3.pack(side="top", pady=5, anchor="w")
    label8 = tk.Label(frame3_3, text="학과", width=6, anchor="w")
    label8.pack(side="left", padx=5)
    combo8 = ttk.Combobox(frame3_3, values=majors2, width=14)
    combo8.pack(side="left", padx=5)
    label9 = tk.Label(frame3_3, text="상태", width=6, anchor="e")
    label9.pack(side="left", padx=5)
    combo9 = ttk.Combobox(frame3_3, values=states, width=6)
    combo9.pack(side="left", padx=5)

    frame3_5 = tk.Frame(frame3)
    frame3_5.pack(side="bottom", pady=5, anchor="w")
    button5 = tk.Button(frame3_5, text="로그아웃", pady=2, width=12, command=lambda: logout(root))
    button5.pack(side="left", padx=5)
    button6 = tk.Button(frame3_5, text="메인 화면", pady=2, width=26, command=lambda: router.route_w2(root))
    button6.pack(side="left", padx=5)

    frame3_4 = tk.Frame(frame3)
    frame3_4.pack(side="bottom", pady=5, anchor="w")
    button2 = tk.Button(frame3_4, text="추가", pady=2, width=12, command=lambda: onclick_add(combo1, entry2, entry3, entry5, entry6, entry7, combo8, combo9, tree1))
    button2.pack(side="left", padx=5)
    button3 = tk.Button(frame3_4, text="저장", pady=2, width=12, command=lambda: onclick_save(tree1, entry5, entry6, entry7, combo8, combo9))
    button3.pack(side="left", padx=5)
    button4 = tk.Button(frame3_4, text="삭제", pady=2, width=12, command=lambda: onclick_delete(entry5, entry6, entry7, combo8, combo9, tree1, button4))
    ...
    def check_for_delete_btn(*args):
        if current_user["role"] != "admin" or not tree1.focus():
            button4.config(state="disabled")
        else:
