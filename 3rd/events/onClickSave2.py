...
def on_click_save2(ipt_current_pw:tk.Entry, ipt_new_pw:tk.Entry, ipt_verify_new_pw:tk.Entry, master:tk.Widget):
    ...
    # 1) 데이터베이스에 있는 암호 수정
    update_user_password(current_user["id"], ipt_new_pw.get())
    # 2) json파일에 있는 암호 수정(현재 사용자의 계정정보가 들어있음)
    modified_user = {
        "id" : current_user["id"],
        "name" : current_user["name"],
        "role" : current_user["role"],
        "password" : ipt_new_pw.get(),
    }
    with open("CurrentUser.json", "w", encoding="utf-8") as f:
        json.dump(modified_user, f, ensure_ascii=False, indent=4)

    # 암호 변경 성공 메세지
    messagebox.showinfo("암호 변경 완료", "암호가 정상적으로 변경되었습니다")

    # 로그인 페이지로 이동
    Router.run_w1(master)
