...
create table if not exists 업무사용자(
    ...
    이름 text not null,
    권한 text not null check (권한 in ("admin", "user")),
    암호 text not null,
    check (length(사번) = 8 and 사번 glob "[0-9]*")
);

-- 데이터 추가
insert or ignore into 업무사용자(사번, 이름, 권한, 암호) 
values
...
-- 데이터 추가
insert or ignore into 학과정보(학과코드, 명칭)
values
("c001", "정보통신과"),
("c002", "AI개발과"),
("c003", "클라우드보안과"),
...
create table if not exists 학생정보 (
    ...
    이메일 text not null,
    학과 text not null,
    상태 text not null check (상태 in ("재학", "졸업", "휴학", "퇴학"))
);

-- 데이터 추가
insert or ignore into 학생정보(학번, 이름, 이메일, 학과, 상태)
values
