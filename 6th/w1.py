...
def main():
    ...
    entry2 = tk.Entry(frame2, width=20, textvariable=entry2_var, validate="key", validatecommand=vcmd, show="*")
    entry2.pack(side="left")

    frame3 = tk.Frame(main)
    frame3.pack(side="top", pady=5, anchor="e")
    button1 = tk.Button(frame3, text="로그인", pady=2, state="disabled", command=lambda: login(entry1, entry2, root))
    button1.pack(side="left", padx=5)
    button2 = tk.Button(frame3, text="종료", pady=2, command=root.quit)
