create table if not exists 학생정보(
    ...
    학과 text not null,
    상태 text not null,
    check (학번 glob "[0-9]*" and length(학번) = 5),
...
create table if not exists 업무사용자 (
    ...
    권한 text not null,
    암호 text not null,
    check (사번 glob "[0-9]*" and length(사번) = 8)
);


insert or ignore into 학생정보 values
('22001', '문소리', 's22001@kead.ac.kr', 'c001', '졸업'),
('22002', '김원중', 's22002@kead.ac.kr', 'c003', '졸업'),
('22003', '김은하', 's22002@kead.ac.kr', 'c002', '휴학'),
...
insert or ignore into 학과정보 values
('c001', '정보통신과'),
('c002', 'AI개발과'),
('c003', '클라우드보안과'),
('c004', '모바일보안과');

insert or ignore into 업무사용자 values
